...
import { Trash2, FilterIcon, X, ChevronDown, ChevronUp } from "lucide-react";
...
export function BatchList({
  ...
  const personaOptions = useMemo(() => {
    const uniquePersonas = Array.from(
      new Set(batches.map((batch) => batch.persona_name))
    );
    ...
    <div className="space-y-2">
        ...
        {(searchName || selectedPersona !== "all" || selectedDate) && (
          <div className="flex gap-1 items-center flex-wrap flex-1">
            {searchName && (
              <Badge
                variant="outline"
                className="bg-[#F1E6EA] text-[#261436] border-[#261436]/20 text-xs"
              >
                {searchName}
                <X
      ...
      {showFilters && (
        <div className="space-y-2 p-2 bg-white/50 rounded-md mb-2">
          <div>
            <Label htmlFor="name-filter" className="text-[#261436] text-xs">
              Name
            </Label>
            <Input
              id="name-filter"
          ...
          <div>
            <Label htmlFor="persona-filter" className="text-[#261436] text-xs">
              Persona
            </Label>
          ...
          <div>
            <Label htmlFor="date-filter" className="text-[#261436] text-xs">
              Date
            </Label>
            <Input
              id="date-filter"
